...
# Preamble
##########
packages <- c('gridExtra', 'kableExtra', 'tidyverse', 'semPlot',
              'lavaan', 'mice', 'readr', 'DataExplorer', 'data.table', 
              'semTools', 'tidyr', 'reshape2', 'psych')
...
########################
dat <- read_csv("code/fsm_analysis/data_publication_material/fsm_raw_deid_wide.csv") %>%
...
long_mi <- data.table::melt(setDT(wide_mi), measure.vars=list(c(1,9), c(2,10), c(3,11), c(4,12), c(5,13), c(6,14), c(7,15), c(8,16),
                                                              ...
                                                              c(31,32), c(33,34), c(35,36), c(37,38), c(39,40), c(43,44)), 
...
# Specifying structural model portion of FSM for both waves
  ...
  relat_child_w1 ~  uae_national + ba_higher_education + s_c_Age + c_Sex  + g1*PatStress_w1 
  relat_child_w2 ~  uae_national + ba_higher_education + s_c_Age + c_Sex  + c11*PatStress_w1 + relat_child_w1 + lb1*PatStress_w2
  ...
  ChildWB_w1 ~  uae_national + ba_higher_education  + s_c_Age + c_Sex + c_outdoor_act_w1  + g2*relat_child_w1 + b2*ParRelat_w1
  ChildWB_w2 ~  uae_national + ba_higher_education  + s_c_Age + c_Sex + c_outdoor_act_w1  + c12*relat_child_w2  + p2*ParRelat_w2 + r1*relat_child_w1 + b12*ParRelat_w1 + ChildWB_w1
...
# Covariances
  ...
  ParRelat_w1 ~~ relat_child_w1 # CALL
  ParRelat_w2 ~~ relat_child_w2 # CALL
...
'
# Fitting the model
wsem_7_fit <- sem(wsem_7, data = wide_c1, estimator = "ML")
summary(wsem_7_fit)
...
# regressions
  ...
  relat_child_w1_c1 ~ uae_national + ba_higher_education + s_age_parent_imp + c_Sex_c1   + b1*PatStress_w1
  relat_child_w1_c2 ~ uae_national + ba_higher_education + s_age_parent_imp + c_Sex_c2  + g1*PatStress_w1
  ChildWB_w1_c1 ~ uae_national + ba_higher_education + c_Sex_c1 + s_c_Age_c1 + c_outdoor_act_w1_c1  + b2*relat_child_w1_c1 + y1*ParRelat_w1
  ChildWB_w1_c2 ~ uae_national + ba_higher_education + c_Sex_c2  + s_c_Age_c2 + c_outdoor_act_w1_c2  + g2*relat_child_w1_c2 + p1*ParRelat_w1 
...
# Covariances
  ...
  relat_child_w1_c1 ~~ relat_child_w1_c2 
...
# Mediation chains
  ...
  '
# Fitting the model
fit_csem_7 <- sem(csem_7, data = c_wide_w1, meanstructure=TRUE, std.lv=TRUE, estimator = "ML")
